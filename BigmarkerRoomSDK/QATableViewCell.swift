...
class QATableViewCell: UITableViewCell {
    ...
    func setupUI(){
        ...
        if question!.answered {
            self.avatarImageView.image = UIImage(named: "answered_check")
        } else {
            var url: URL!
            if (question!.userAvatar.range(of: "assets") != nil) {
                url = URL(string: SERVICE_API_DOMAIN + question!.userAvatar)
            } else {
        ...
        if question!.voted {
            self.voteImageView.image = UIImage(named: "poll_voted")
            self.numberLabel.textColor = UIColor.init(red: 0/255, green: 191/255, blue: 130/255, alpha: 1)
        } else {
            self.voteImageView.image = UIImage(named: "poll_unvoted")
