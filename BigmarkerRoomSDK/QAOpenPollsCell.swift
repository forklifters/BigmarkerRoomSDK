...
class QAOpenPollsCell: UITableViewCell {
...
extension QAOpenPollsCell{
    ...
    func setupUI(pollsModel:Poll,indexPath:NSIndexPath){
        ...
        for i in 0...(answerArr.count - 1){
            ...
            let answerLabelHeight = getLabelHeight(answerLabelText: pollChoiceModel.choice, font: UIFont.systemFont(ofSize: 12), width: ScreenW - 50) + 10
            ...
            let answerButton = setupAnswerButton(answerButtonFrame: CGRect(x: 0, y: 0, width: 20, height: 20),index: i)
    ...
    func setupAnswerButton(answerButtonFrame:CGRect,index:Int)->UIButton{
        ...
        if isSingle == "multi" {
            button.setImage(UIImage(named: "multiUnSelect"), for: .normal)
            button.setImage(UIImage(named: "multiSelect"), for: .selected)
        }else{
            button.setImage(UIImage(named: "QAPollsUnselectedBtn"), for: .normal)
            button.setImage(UIImage(named: "QAPollsSelectedBtn"), for: .selected)
        }
