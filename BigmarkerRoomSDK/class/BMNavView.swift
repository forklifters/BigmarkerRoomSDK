...
class BMNavView: UIView {
    ...
    func setupUI(){
        ...
        self.titleLabel?.text = self.conference?.channel?.name
    ...
    func clickButton(sender: AnyObject){
        ...
        let audio = PopoverAction.init(title: "Audio Only", handler: {action in
        ...
        let uiSwitch = UISwitch.init(frame: CGRect.init(x: 10, y: 18, width: 13, height: 3))
