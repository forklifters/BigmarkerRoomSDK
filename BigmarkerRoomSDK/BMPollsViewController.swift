...
private let QAOpenPollsCellId = "QAOpenPollsCell"
private let QAClosedPollsCellId = "QAClosedPollsCell"
...
extension BMPollsViewController{
    ...
    func setupUI(){
        ...
        tableView.register(UINib(nibName: "QAOpenPollsCell",bundle: nil), forCellReuseIdentifier: QAOpenPollsCellId)
        tableView.register(UINib(nibName: "QAClosedPollsCell",bundle: nil), forCellReuseIdentifier: QAClosedPollsCellId)
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if (statusView.type == .Active){
            ...
            if bigRoomPollsViewModel.openPoll.count == 0 {
                ...
                cell.resultImageView.image = UIImage(named: "iconPolls")
                ...
                if admin == true {
                    //是管理员
                    let cell : QAClosedPollsCell = tableView.dequeueReusableCell(withIdentifier: QAClosedPollsCellId, for: indexPath) as! QAClosedPollsCell
                    ...
                    if poll.isSelfVoted(currentUserId: BMCurrentUser.id()) {
                        let cell : QAClosedPollsCell = tableView.dequeueReusableCell(withIdentifier: QAClosedPollsCellId, for: indexPath) as! QAClosedPollsCell
                        cell.setupClosedPollModel(closedPollsModel: bigRoomPollsViewModel.openPoll[indexPath.row],admin: admin,type:QAType.Active)

                        return cell
                    }else{
                        let cell : QAOpenPollsCell = tableView.dequeueReusableCell(withIdentifier: QAOpenPollsCellId, for: indexPath) as! QAOpenPollsCell
